...
const SHARED_SECRET = '$ItG4@^7Kb1ZI5QPVfybRmDXL&A*uWY@GRatQf!s1evkSFml';   // 与前端 SHARED_SECRET 相同
const TARGET_FROM = [
  'otp@tm1.openai.com',
...
/**
 * extractCode_
 * Try subject first, then plain/html body.
 */
function extractCode_(subject, plainBody, htmlBody) {
  const fromSubject = lastSixDigitsFromSubject_(subject);
  if (fromSubject) return fromSubject;

  const plainText = (plainBody || '').replace(/\r/g, '');
  const htmlText = htmlToVisibleText_(htmlBody || '');

  const fromPlainByKeyword = sixDigitsNearKeyword_(plainText);
  if (fromPlainByKeyword) return fromPlainByKeyword;

  const fromHtmlByKeyword = sixDigitsNearKeyword_(htmlText);
  if (fromHtmlByKeyword) return fromHtmlByKeyword;

  const fromPlain = firstStandaloneSixDigits_(plainText);
  if (fromPlain) return fromPlain;

  return firstStandaloneSixDigits_(htmlText);
}
...
function findCodeInWindow_(sinceEpoch, winSec) {
  ...
  for (const th of threads) {
    ...
    for (const m of msgs) {
      ...
      const code = extractCode_(subject, plainBody, htmlBody);
      if (!code) continue; 
      // 条件成立：标题和正文都未提取到 6 位验证码 → 跳过
